...
#include "../pocorocore/async_auto_reset_event.hpp"
...
pocoro::async_auto_reset_event::async_auto_reset_event(bool initiallySet) noexcept
...
pocoro::async_auto_reset_event::~async_auto_reset_event()
{
	assert(m_newWaiters.load(std::memory_order_relaxed) == nullptr);
	assert(m_waiters == nullptr);
}

pocoro::async_auto_reset_event_operation
pocoro::async_auto_reset_event::operator co_await() const noexcept
{
...
void pocoro::async_auto_reset_event::set() noexcept
{
	std::uint64_t oldState = m_state.load(std::memory_order_relaxed);
	do
	{
...
void pocoro::async_auto_reset_event::reset() noexcept
{
	std::uint64_t oldState = m_state.load(std::memory_order_relaxed);
...
void pocoro::async_auto_reset_event::resume_waiters(
	std::uint64_t initialState) const noexcept
{
	...
	do
	{
		...
		for (std::uint32_t i = 0; i < waiterCountToResume; ++i)
		{
			if (m_waiters == nullptr)
			{
				...
				POCORO_ASSUME(newWaiters != nullptr);
	...
	POCORO_ASSUME(waitersToResumeList != nullptr);
...
pocoro::async_auto_reset_event_operation::async_auto_reset_event_operation() noexcept
	: m_event(nullptr)
{}

pocoro::async_auto_reset_event_operation::async_auto_reset_event_operation(
...
bool pocoro::async_auto_reset_event_operation::await_suspend(
