...
#include "../pocorocore/detail/lightweight_manual_reset_event.hpp"
...
# if POCORO_OS_WINNT >= 0x0602

pocoro::detail::lightweight_manual_reset_event::lightweight_manual_reset_event(bool initiallySet)
	: m_value(initiallySet ? 1 : 0)
{}

pocoro::detail::lightweight_manual_reset_event::~lightweight_manual_reset_event()
{
}

void pocoro::detail::lightweight_manual_reset_event::set() noexcept
{
	m_value.store(1, std::memory_order_release);
	::WakeByAddressAll(&m_value);
}

void pocoro::detail::lightweight_manual_reset_event::reset() noexcept
{
	m_value.store(0, std::memory_order_relaxed);
}

void pocoro::detail::lightweight_manual_reset_event::wait() noexcept
{
