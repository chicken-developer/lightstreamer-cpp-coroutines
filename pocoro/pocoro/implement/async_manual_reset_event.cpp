...
#include "../pocorocore/async_manual_reset_event.hpp"
#include "../pocorocore/config.hpp"

#include <cassert>

pocoro::async_manual_reset_event::async_manual_reset_event(bool initiallySet) noexcept
	: m_state(initiallySet ? static_cast<void*>(this) : nullptr)
{}

pocoro::async_manual_reset_event::~async_manual_reset_event()
{
...
bool pocoro::async_manual_reset_event::is_set() const noexcept
{
	return m_state.load(std::memory_order_acquire) == static_cast<const void*>(this);
}

pocoro::async_manual_reset_event_operation
pocoro::async_manual_reset_event::operator co_await() const noexcept
{
	return async_manual_reset_event_operation{ *this };
}

void pocoro::async_manual_reset_event::set() noexcept
{
...
void pocoro::async_manual_reset_event::reset() noexcept
{
	void* oldState = static_cast<void*>(this);
	m_state.compare_exchange_strong(oldState, nullptr, std::memory_order_relaxed);
}

pocoro::async_manual_reset_event_operation::async_manual_reset_event_operation(
	const async_manual_reset_event& event) noexcept
	: m_event(event)
{
}

bool pocoro::async_manual_reset_event_operation::await_ready() const noexcept
{
	return m_event.is_set();
}

bool pocoro::async_manual_reset_event_operation::await_suspend(
